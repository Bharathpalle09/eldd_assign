...
static __init int pchar_init(void)
{
	...
	if(ret<0)
	{
		printk(KERN_INFO "%s alloc_chrdev() is failed\n ",THIS_MODULE->name);
	return ret;
	}
	minor=MINOR(devno);
	major=MAJOR(devno);
	printk(KERN_INFO " %s alloc_chrdev () -allocated devno= %d-%d\n",THIS_MODULE->name,major,minor);

	pclass=class_create("pchar_class");
		if(IS_ERR(pclass))
		{
			printk(KERN_INFO "%s create classe is failed\n",THIS_MODULE->name);
			unregister_chrdev_region(devno,1);
			return -1;
		}
	printk(KERN_INFO "%s :class_create is created \n",THIS_MODULE->name);
	...
	printk(KERN_INFO "%s device create() - device is created\n",THIS_MODULE->name);

	cdev_init(&cdev,&f_ops);
	ret=cdev_add(&cdev,devno,1);
	if(ret<0)
	{
	printk(KERN_INFO "%s cdev is failed \n",THIS_MODULE->name);
	device_destroy(pclass,devno);
	class_destroy(pclass);
	unregister_chrdev_region(devno,1);
	}
	printk(KERN_INFO "%s cdev() is -device is created\n",THIS_MODULE->name);
...
static __exit void pchar_exit(void)
{
	...
	cdev_del(&cdev);
	printk(KERN_INFO "%s cdev is del successfully \n",THIS_MODULE->name);
	device_destroy(pclass,devno);
	printk(KERN_INFO "%s :device  is destroyed\n",THIS_MODULE->name); 
	
	class_destroy(pclass);
	printk(KERN_INFO "%s :class is deallocated\n",THIS_MODULE->name);
	
       unregister_chrdev_region(devno,1);
			printk(KERN_INFO "%s :unrigestred _chrdev_region() completed\n",THIS_MODULE->name);
...
static ssize_t pchar_read(struct file *pfile,char __user *buf,size_t size,loff_t *poffset);
...
if(bytes_to_read == 0)
{
printk(KERN_ERR"%s :pchar_read() : nodata available\n",THIS_MODULE->name);
return -1;
}
...
MODULE_DESCRIPTION("THIS IS FIRST PROGRAM");
